"""Raygent SDK project profile embedding with a fake model provider.
...
from raygent_harness.core.query_engine import SDKAssistantMessage, SDKResult, SDKSystemInit
from raygent_harness.sdk import RaygentToolProfileOptions, create_raygent
...
class StaticModelProvider:
    async def complete(self, request: ModelRequest) -> ModelResponse:
        ...
        return model_response_from_message_param(
            assistant_message(f"Project profile is ready. Model-visible tools: {tool_names}")
        )
...
async def main() -> None:
    session = create_raygent(
        ...
        model="demo-model",
        session_id="project-profile-session",
        tools="project",
        context="project",
        tool_profile_options=RaygentToolProfileOptions(enable_bash=False),
    )

    async for event in session.run("Summarize the project profile setup."):
